...
export default function App() {
  ...
  const [showAddEditVehicleModal, setShowAddEditVehicleModal] = useState(false);
      ...
      if (editingVehicleId) {
        ...
        if (newVehiclePlate && vehicles && vehicles.length > 0) {
          ...
          if (targetVehicle) {
            ...
            setShowVehicleModal(false);
            setShowAddEditVehicleModal(false);
            setShowSuggestions(false);
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col w-full max-w-md sm:max-w-xl md:max-w-2xl mx-auto relative shadow-2xl border-x border-slate-900 transition-all duration-300">
      ...
      {/* Vehicle Manager Modal - Chỉ hiển thị danh sách xe và nút mở Modal Thêm mới */}
      {showVehicleModal && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-card rounded-3xl w-full max-w-sm p-6 space-y-5 animate-fade-in">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">Chọn xe đang dùng hoặc thêm xe mới</p>
              </div>
              <button 
                onClick={() => {
                  setShowVehicleModal(false);
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto pr-0.5">
              <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider pl-1">
                Danh sách xe của bạn ({vehicles?.length || 0})
              </p>
              {vehicles?.map(v => (
                <div 
                  key={v.id} 
                  onClick={() => handleSelectVehicle(v.id)}
                  className={`bg-slate-900 border rounded-2xl p-3.5 flex justify-between items-center cursor-pointer transition active:scale-98 ${
                ...
                >
                  <div className="flex-1 min-w-0 pr-2">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      Biển số: <span className="text-slate-300 font-medium">{v.plateNumber || 'Chưa có'}</span> • Bình xăng: <span className="text-emerald-400 font-semibold">{v.tankCapacity ? `${v.tankCapacity}L` : 'Chưa nhập'}</span>
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <button
                      ...
                      className="text-[11px] text-sky-400 hover:text-sky-300 font-bold px-2.5 py-1.5 hover:bg-slate-800 rounded-lg transition active:scale-95"
                    >
                      Sửa
                    </button>

                    {v.id.toString() === currentVehicleId ? (
                      <span className="text-[9px] font-bold text-brand-400 bg-brand-500/10 px-2 py-1 rounded-md border border-brand-500/20">
                        Đang chọn
                      </span>
                    ) : (
                      vehicles.length > 1 && (
            ...
            {/* Nút mở Modal thêm phương tiện mới */}
            <button
              type="button"
              onClick={() => {
                setEditingVehicleId(null);
              ...
              className="w-full bg-gradient-to-r from-brand-500 to-emerald-600 hover:from-brand-600 hover:to-emerald-700 text-white font-bold py-3 px-4 rounded-2xl shadow-lg shadow-brand-500/20 flex items-center justify-center gap-2 active:scale-95 transition-all duration-150 text-sm"
            >
      ...
      {/* Dedicated Add/Edit Vehicle Modal - Mở riêng biệt, tối ưu cho bàn phím Mobile */}
      {showAddEditVehicleModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-start sm:items-center justify-center p-4 overflow-y-auto pt-6 sm:pt-4">
          <div className="glass-card rounded-3xl w-full max-w-sm p-5 sm:p-6 space-y-4 animate-fade-in my-auto border-brand-500/20 shadow-2xl">
            <div className="flex items-center justify-between pb-1 border-b border-slate-900">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-100 flex items-center gap-1.5">
                  ...
                  <span>{editingVehicleId ? 'Sửa thông tin xe' : 'Thêm phương tiện mới'}</span>
                </h3>
              </div>
              <button 
                type="button"
            ...
            {!editingVehicleId && (
              <div className="space-y-1.5">
                <p className="text-[10px] text-slate-400 font-medium pl-1 flex items-center gap-1">
                  ...
                  <span>Chọn nhanh mẫu xe phổ biến (1 chạm):</span>
                </p>
            ...
            <form onSubmit={handleAddVehicle} className="space-y-3 pt-1">
              <div className="flex flex-col gap-2.5">
                {/* Ô Tên xe với Autocomplete */}
                <div className="relative">
                  <label className="text-xs text-slate-400 font-medium mb-1 pl-1 block">Tên phương tiện *</label>
                  <input
                    type="text"
                    placeholder="Ví dụ: Honda Winner X, SH, Vios..."
                    ...
                    className="glass-input text-sm py-2.5 w-full pr-8"
                    required
                  />
                  ...
                  {newVehicleName && (
                    <button
                      ...
                      className="absolute right-2.5 top-8 p-0.5 text-slate-500 hover:text-slate-300 transition"
                      title="Xóa nội dung"
                    >
                  ...
                  {showSuggestions && vehicleSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-slate-900/98 backdrop-blur-md border border-slate-700/90 rounded-2xl shadow-2xl overflow-hidden animate-fade-in divide-y divide-slate-800/80 max-h-52 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-slate-400 font-medium mb-1 pl-1 block">Loại xe</label>
                    <select
                  ...
                  <div>
                    <label className="text-xs text-slate-400 font-medium mb-1 pl-1 block">Biển số xe</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="text-xs text-slate-400 font-medium mb-1 pl-1 block">Dung tích bình xăng</label>
                  <div className="relative">
                    <input
                      type="number"
                      step="0.1"
