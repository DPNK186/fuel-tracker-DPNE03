...
### Added (Tính năng mới & Trải nghiệm Đồng bộ)

- **Cơ chế Đối soát Phương tiện 2 Chiều khi Đồng bộ Google Drive:**
  - Ngăn chặn hoàn toàn lỗi xe đã xóa bị "hồi sinh" sau khi đồng bộ với đám mây.
  - Tích hợp **Modal Đối soát Phương tiện** trực quan xuất hiện trước khi đồng bộ hoặc khôi phục nếu phát hiện chênh lệch danh sách xe giữa Thiết bị và Cloud:
    - **Chiều A (Có trên Cloud • Thiếu ở máy):** Cho phép người dùng chọn **"Xóa trên Cloud"** (xóa triệt để cả xe và lịch sử liên quan trên đám mây) hoặc **"Khôi phục về máy"** (tải lại dữ liệu từ Cloud về máy).
    - **Chiều B (Có trên máy • Thiếu trên Cloud):** Cho phép người dùng chọn **"Xóa trên máy"** (đồng bộ theo thao tác xóa từ thiết bị khác) hoặc **"Tải lên Cloud"** (giữ lại và sao lưu lên đám mây).
  - **Tự động gợi ý thông minh dựa trên mốc thời gian (Smart Suggestions):**
    - Tự động so sánh mốc xóa xe `deletedAt` trên máy với mốc thời gian bản sao lưu `cloudTimestamp` để gợi ý chính xác hành động tối ưu kèm lý do trực quan.
    - Tự động tích chọn sẵn hành động gợi ý giúp người dùng hoàn tất đối soát chỉ với 1 chạm.
    - Cung cấp các thao tác đối soát hàng loạt nhanh: *"Áp dụng gợi ý"*, *"Giữ lại tất cả xe"*, *"Đồng bộ theo máy này"*.
  - **Tương thích ngược 100% với dữ liệu cũ (Legacy Data):**
    - Tự động dò tìm và ước tính mốc thời gian từ bản ghi giao dịch (đổ xăng / chi phí) gần nhất nếu xe hoặc bản sao lưu cũ chưa có trường `timestamp`/`updatedAt`.
    - Nguyên tắc bảo toàn dữ liệu: Luôn mặc định ưu tiên *"Tải lên Cloud"* đối với các xe sẵn có từ phiên bản trước trên thiết bị, tuyệt đối không tự ý gợi ý xóa xe cũ của người dùng.
    - Tự động gán `updatedAt` khi nhập dữ liệu vào Dexie DB hoặc hợp nhất lên Cloud, nâng cấp liền mạch cơ sở dữ liệu cũ lên chuẩn mới.
  - **Định dạng thời gian an toàn:** Bổ sung các helper xử lý chuỗi ngày giờ an toàn, phòng ngừa triệt để lỗi crash hoặc hiển thị `Invalid Date`.

### Fixed (Sửa lỗi)

- Dọn dẹp triệt để các lệnh gọi `autoBackup` cũ trong `App.jsx` khi xóa xe hoặc giải quyết xung đột, ngăn ngừa lỗi runtime tiềm ẩn.
